...
namespace ComponentDesgin
{
    public partial class Hand
    {
        ...
        {
            int maxMjCount = desk.GetDaChuMjRowMaxMjCount();
            handActionPoints = new float[fit.deskRowMjLayoutCounts.Length, maxMjCount, (int)ActionCombineNum.End];
            pointActionCombineNumLists = new List<ActionCombineNum>[fit.deskRowMjLayoutCounts.Length, maxMjCount];
        ...
        void CreateHands(PlayerType handStyle, HandDirection handDir)
        {
            ...
            GameObject prefabHand = mjAssetsMgr.defaultPrefabDict[(int)PrefabIdx.FEMALE_HAND][0];
            ...
            for (int i = 0; i < 4; i++)
            {
                ...
                cpHand.transform.SetParent(scene.root, false);
        ...
        void SaveHand(int seat, PlayerType handStyle, HandDirection handDir, GameObject saveHand)
        {
            if (handStyle == PlayerType.FEMALE)
            {
                if (handDir == HandDirection.LeftHand)
                {
                    ...
                    if (mjAssetsMgr.handActionDataInfoDicts.ContainsKey(saveHand.name))
                        femaleLHandsActionDataInfo[seat] = mjAssetsMgr.handActionDataInfoDicts[saveHand.name];
                }
                else
                {
                    ...
                    if (mjAssetsMgr.handActionDataInfoDicts.ContainsKey(saveHand.name))
                        femaleRHandsActionDataInfo[seat] = mjAssetsMgr.handActionDataInfoDicts[saveHand.name];
                }
            }
            else
            {
                if (handDir == HandDirection.LeftHand)
                {
                    ...
                    if (mjAssetsMgr.handActionDataInfoDicts.ContainsKey(saveHand.name))
                        maleLHandsActionDataInfo[seat] = mjAssetsMgr.handActionDataInfoDicts[saveHand.name];
                }
                else
                {
                    ...
                    if (mjAssetsMgr.handActionDataInfoDicts.ContainsKey(saveHand.name))
                        maleRHandsActionDataInfo[seat] = mjAssetsMgr.handActionDataInfoDicts[saveHand.name];
                }
            }
        }
        ...
        /// <returns></returns>
        public GameObject GetHandBone(int seat, PlayerType handStyle, HandDirection handDir, int boneIdx)
        {
            if (handStyle == PlayerType.FEMALE)
            {
                if (handDir == HandDirection.LeftHand)
        ...
        public GameObject GetHandShadowAxis(int seat, PlayerType handStyle, HandDirection handDir, int shadowIdx)
        {
            if (handStyle == PlayerType.FEMALE)
            {
                if (handDir == HandDirection.LeftHand)
        ...
        /// <param name="handDir"></param>
        /// <returns></returns>
        public Dictionary<string, ActionDataInfo> GetHandActionDataDict(int seat, PlayerType handStyle, HandDirection handDir)
        {
            if (handStyle == PlayerType.FEMALE)
            {
                if (handDir == HandDirection.LeftHand)
        ...
        /// <param name="handDir"></param>
        /// <returns></returns>
        public GameObject GetHand(int seat, PlayerType handStyle, HandDirection handDir)
        {
            if (handStyle == PlayerType.FEMALE)
            {
                if (handDir == HandDirection.LeftHand)
        ...
        /// <param name="handDir"></param>
        /// <returns></returns>
        public Animation GetHandAnimation(int seat, PlayerType handStyle, HandDirection handDir)
        {
            if (handStyle == PlayerType.FEMALE)
            {
                if (handDir == HandDirection.LeftHand)
        ...
        /// <returns></returns>
        public float GetHandActionWaitTime(int seatIdx, PlayerType handStyle, HandDirection handDir, string actionName)
        {
            Dictionary<string, ActionDataInfo> actionDataDict = GetHandActionDataDict(seatIdx, handStyle, handDir);
            Animation anim = GetHandAnimation(seatIdx, handStyle, handDir);
        ...
        public float MoveHandToDstDirRelative(int seatIdx, PlayerType handStyle, HandDirection handDir, Vector3 endPos, float moveScale = 12f)
        {
            //
        ...
        /// <param name="handDir"></param>
        /// <returns></returns>
        public List<Vector3> GetDeskMjHandOffsetList(int seat, PlayerType handStyle, HandDirection handDir)
        {
            if (handStyle == PlayerType.FEMALE)
            {
                if (handDir == HandDirection.LeftHand)
        ...
        public ActionCombineNum GetRandomHandDaPaiActionNumForNextDeskMjPos(int seatIdx)
        {
            ...
            Vector3Int posIdx = desk.GetNextDeskMjPosIdx(seatIdx);
